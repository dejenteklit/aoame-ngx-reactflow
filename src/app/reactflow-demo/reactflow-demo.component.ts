import { Component, ViewChild ,OnInit, ElementRef} from '@angular/core';
import { ReactFlowComponent } from 'ngx-reactflow';
import { MarkerType, Position } from 'reactflow';
import React from "react";



@Component({
  selector: 'app-reactflow',
  templateUrl: './reactflow-demo.component.html',
  styleUrls: ['./reactflow-demo.component.css'],
})
export class ReactflowDemoComponent implements OnInit {

  //@ViewChild('reactFlow') reactFlow: ElementRef<ReactFlowComponent> | undefined;
  //@ViewChild('reactFlow') reactFlow: ReactFlowInstance | undefined;

  /*nodes: Node[] = [
    {id: '1', position: {x: 0, y: 0}, data: {label: '1'}},
    {id: '2', position: {x: 0, y: 100}, data: {label: '2'}},
    {id: '3', position: {x: 0, y: 150}, data: {label: '3'}}
  ];
  edges: Edge[] = [
    {id: 'e1-2', source: '1', target: '2'},
    {id: 'e2-3', source: '2', target: 'null'}
  ];
  /////////////////////////////////
  nodes: Node[] = [
    {id: '1', position: {x: 0, y: 0}, data: {label: '1'}},
    {id: '2', position: {x: 0, y: 100}, data: {label: '2'}},
    {id: '3', position: {x: 0, y: 150}, data: {label: '3'}}
  ];
  edges: Edge[] = [{id: 'e1-2', source: '1', target: '2'},
    {id: 'e2-2', source: '2', target: 'null'}
  ];

  onConnect(params: Connection[]) {
    params.forEach(connection => {
      this.edges = addEdge(connection, this.edges);
    });
  }

  ngOnInit() {
    setTimeout(() => {
      this.nodes = [...this.nodes];
      this.edges = [...this.edges];
    }, 4000)
  }
}
  */

  nodes = [
    {
      id: '1',
      type: 'input',
      data: {
        label: 'Input Node',
      },
      draggable: true,
      selectable: true,
      position: {x: 250, y: 0},
    },
    {
      id: '2',
      data: {
        label: 'Default Node',
      },
      draggable: true,
      selectable: true,
      position: {x: 100, y: 100},
    },
    {
      id: '3',
      type: 'output',
      data: {
        label: 'Output Node',
      },
      position: {x: 400, y: 100},
    },
    {
      id: '4',
      type: 'custom',
      position: {x: 100, y: 200},
      draggable: true,
      selectable: true,
      data: {
        selects: {
          'handle-0': 'smoothstep',
          'handle-1': 'smoothstep',
        },
      },
    },
    {
      id: '5',
      type: 'output',
      data: {
        label: 'custom style',
      },
      className: 'circle',
      style: {
        background: '#2B6CB0',
        color: 'white',
      },
      draggable: true,
      selectable: true,
      position: {x: 400, y: 200},
      sourcePosition: Position.Right,
      targetPosition: Position.Left,
    },
    {
      id: '6',
      type: 'output',
      style: {
        background: '#63B3ED',
        color: 'white',
        width: 100,
      },
      data: {
        label: 'Node',
      },
      draggable: true,
      selectable: true,
      position: {x: 400, y: 325},
      sourcePosition: Position.Right,
      targetPosition: Position.Left,
    },
    {
      id: '7',
      type: 'default',
      className: 'annotation',
      data: {
        label: React.createElement(
          'div',
          null,
          'On the bottom left you see the ',
          React.createElement(
            'strong',
            null,
            'Controls'
          ),
          ' and the bottom right the',
          ' ',
          React.createElement(
            'strong',
            null,
            'MiniMap'
          ),
          '. This is also just a node \uD83E\uDD73'
        )
      },
      draggable: true,
      selectable: true,
      position: {x: 150, y: 400},
    },
  ];

  edges = [
    {id: 'e1-2', source: '1', target: '2', label: 'this is an edge label'},
    {id: 'e1-3', source: '1', target: '3', animated: true},
    {
      id: 'e4-5',
      source: '4',
      target: '5',
      type: 'smoothstep',
      sourceHandle: 'handle-0',
      data: {
        selectIndex: 0,
      },
      markerEnd: {
        type: MarkerType.ArrowClosed,
      },
    },
    {
      id: 'e4-6',
      source: '4',
      target: '6',
      type: 'smoothstep',
      sourceHandle: 'handle-1',
      data: {
        selectIndex: 1,
      },
      markerEnd: {
        type: MarkerType.ArrowClosed,
      },
    },
  ];

  ngOnInit() {
    setTimeout(() => {

      this.nodes = [...this.nodes];
      this.edges = [...this.edges];
    }, 4000)
  }
}
